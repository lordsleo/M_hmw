    ...
    public partial class VehicleRegistration : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                var sql =
                    string.Format(
                        "select count(id) as sum  from Transit.s_vehicle_br where VEHICLE like '%{0}%'", Vehicle);
                ...
                sql =
                    string.Format(
                        "select id,VEHICLE,VEH_CLASS_TYPE,VEH_TYPE,USE_KIND,BRAND_CODE,SELF_WEIGHT,LOAD_WEIGHT,card_no,mark_validname,TypeName,VEH_CARD, OWNERNAME,OWNER_PHONE,emark,INSPECT_PERIOD,to_char( INPUT_TIME,'yy-MM-dd') from(select * from (select ID,VEHICLE,VEH_CLASS_TYPE,VEH_TYPE,USE_KIND,BRAND_CODE,SELF_WEIGHT,LOAD_WEIGHT,card_no,mark_validname,TypeName,VEH_CARD, OWNERNAME,OWNER_PHONE,emark,INSPECT_PERIOD,INPUT_TIME  from Transit.s_vehicle_br where VEHICLE like '%{0}%' order by INPUT_TIME asc) where rownum<='{1}' order by INPUT_TIME desc) where rownum<='{2}'",
