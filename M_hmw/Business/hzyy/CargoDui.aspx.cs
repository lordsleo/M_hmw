...
{
    public partial class CargoDui : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                sql =
                    string.Format(
                        "select count(code_client) as sum  from vw_stockdui_web where code_client='{0}'", clientCode);
                ...
                sql =
                    string.Format(
                        "select * from(select  ID,department,operatetype,Cargo,inout,vgdisplay,amount,weight,pieceweight,to_char( signdate,'yy-MM-dd')  from (select ID,department,operatetype,Cargo,inout,vgdisplay,amount,weight,pieceweight,SignDate  from vw_stockdui_web where code_client='{0}' order by signdate asc) where rownum<='{1}' order by signdate desc) where rownum<='{2}'",
