...
{
    public partial class VehicleTransport : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                sql =
                    string.Format(
                        "select count(code_client) as sum from((select CODE_CLIENT,code_department from vw_web_consign_query)union all(select CODE_CLIENT,code_department from vw_qyf_consign)) where code_department='{0}' and code_client='{1}'", companyCode, clientCode);
                ...
                sql =
                    string.Format(
                        "select cgno,OperateType,code_client,client,code_department,department,PlanAmount,PlanWeight,PlanVolume,TaskNo,Blno,to_char( createtime,'yy-MM-dd'),complete_mark,complete_mark1,sign_mark,net from(select * from (select * from((select OPERATETYPE,CLIENT,complete_mark,DEPARTMENT,PLANAMOUNT,PLANWEIGHT,PLANVOLUME,CREATETIME,TASKNO,CODE_DEPARTMENT,CODE_CLIENT,BLNO,CGNO,sign_mark, case sign_mark when '0' then '' when '1' then '√' end as net,case complete_mark when '0' then '' when '1' then '√' end as complete_mark1 from vw_web_consign_query)union all(select OPERATETYPE,CLIENT,complete_mark,DEPARTMENT,PLANAMOUNT,PLANWEIGHT,PLANVOLUME,CREATETIME,TASKNO,CODE_DEPARTMENT,CODE_CLIENT,BLNO,CGNO,net_mark, case net_mark when '0' then '' when '1' then '√' end as net,case complete_mark when '0' then '' when '1' then '√' end as complete_mark1 from vw_qyf_consign)) where code_department='{0}' and code_client='{1}' order by createtime asc ) where rownum<='{2}' order by createtime desc) where rownum<='{3}'",
