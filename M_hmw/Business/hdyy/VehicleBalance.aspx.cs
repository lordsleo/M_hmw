...
{
    public partial class VehicleBalance : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                sql =
                    string.Format(
                        "select id,to_char( signdate,'yy-MM-dd'),client,vgdisplay,cargo,inout,trade,taskno,cgno,pack,mark,department from(select * from (select id,signdate,client,vgdisplay,cargo,inout,trade,taskno,cgno,pack,mark,department from vw_gjp_consign where code_department='{0}' and code_client='{1}' order by signdate asc) where rownum<='{2}' order by signdate desc) where rownum<='{3}'",
