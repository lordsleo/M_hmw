...
{
    public partial class ForeShip : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                sql =
                    string.Format(
                            "select count(ship_id) as sum from view_sship_tan where agent='{0}' and ship_statu=0", clientCode);
                ...
                sql =
                    string.Format(
                        "select ship_id,chi_vessel,to_char( yjdg,'yy-MM-dd'),zhmc,zhsl,xhmc,xhsl,ys from(select * from (select ship_id,chi_vessel,yjdg,zhmc,zhsl,xhmc,xhsl,ys from view_sship_tan where agent='{0}' and ship_statu=0 order by yjdg asc) where rownum<='{1}' order by yjdg desc) where rownum<='{2}'",
