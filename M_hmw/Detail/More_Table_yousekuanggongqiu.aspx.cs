...
{
    public partial class More_Table_yousekuanggongqiu : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                sql =
                    string.Format(
                        "select ID,TYPE_NAME,CLASS,SPEC,TONS,PLACE,to_char(REG_DATE,'yyyy-MM-dd') as REG_DATE from(select * from (select id,class,spec,place,tons,type_name,price,reg_date from view_trade_business where type=3 and isnew=1 order by reg_date asc) where rownum<='{0}' order by reg_date desc) where rownum<='{1}'",
            ...
            {
                LogTool.WriteLog(typeof(More_Table_yousekuanggongqiu), ex);
            }
        }
