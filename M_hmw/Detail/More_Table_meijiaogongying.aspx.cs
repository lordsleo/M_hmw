...
{
    public partial class More_Table_meijiaogongying : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                var sql =
                    string.Format(
                        "select count(id) as sum from vw_sup_dem where type='供货'  ");
                ...
                sql =
                    string.Format(
                        "select ID,GOODSNAME,NUM,AREA,to_char(CREATETIME,'yyyy-MM-dd') as CREATETIME from(select * from (select ID,GOODSNAME,NUM,AREA,CREATETIME,audittime from vw_sup_dem where type='供货'   order by audittime asc) where rownum<='{0}' order by audittime desc) where rownum<='{1}'",
