...
{
    public partial class More_News_tiekuangshahangqing : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                var sql =
                    string.Format(
                        "Select count(id) as sum From NEWS_TOPIC Where classid=102");
                ...
                sql =
                    string.Format(
                        "select classid,ID,topic,to_char(POST_TIME,'yyyy-MM-dd') as POST_TIME  from(select * from (Select classid,ID,topic,POST_TIME From NEWS_TOPIC Where classid=102 order by POST_TIME asc) where rownum<='{0}' order by POST_TIME desc) where rownum<='{1}'",
            ...
            {
                LogTool.WriteLog(typeof(More_News_gongluzixun), ex);
            }
        }
